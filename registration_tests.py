...
def combining_transforms():
    ...
    X_rot = reg.shear(0.1,0.2).dot(reg.rotate(np.pi/2+1)).dot(X)
    X_shear = reg.shear(0.1,0.2).dot(reg.reflect(-1,1)).dot(X)
    X_reflect = reg.rotate(np.pi/4).dot(reg.reflect(1,-1)).dot(X)
...
def arbitrary_rotation():
    ...
    T_trans1 = util.t2h(reg.identity(), Xt)
    #T_rot1 = util.c2h(reg.rotate(np.pi/4))
    T_rot1 = util.t2h(reg.rotate(np.pi/4),[0,0])    
    T_trans2 = util.t2h(reg.identity(), -1*Xt)   
    print(T_trans1)
    print(T_rot1)
    print(T_trans2)
    T = T_trans1.dot(T_rot1).dot(T_trans2)
    print(T)
...
def ls_solve_test():
    ...
    print("Maximum error imdex:"+str(np.argmax(A.dot(w)-b)))
    print("W:"+str(w))
...
def correlation_test():
    ...
    I = plt.imread('C:\\Users\\20203226\\Documents\\Boeken\\Q1\\8DC00\\data\\cameraman.tif')
    Th = util.t2h(reg.identity(), np.array([10,20]))
    J, Xt = reg.image_transform(I, Th)
    
    Th2 = util.t2h(reg.shear(0.1,0.1), np.array([10,20]))
    J2, Xt2 = reg.image_transform(I, Th2)
    
    Th3 = util.t2h(reg.rotate(np.pi/4), np.array([10,20]))
    J3, Xt3 = reg.image_transform(I, Th3)
...
def ngradient_test():
    ...
    exponential2 = lambda x: np.exp(2*x)
    g2 = reg.ngradient(exponential2, np.ones((1,)))
    print(g2)
    print(2*exponential(2))
    print("\n")
    assert abs(g2- 2*exponential(2)) < 1e-5, "Numerical gradient is incorrectly implemented (exponential test)"
    
    func2 = lambda x: x**2 +3
    g3 = reg.ngradient(func2, np.array([[3,10,6]]))
    print(g3)
    print('Test successful!')
...
def registration_metrics_demo(use_t2=False):
    ...
    # read a T1 image
    I = plt.imread('C:\\Users\\20203226\\Documents\\Boeken\\Q1\\8DC00\\data\\t1_demo.tif')

    if use_t2:
        # read the corresponding T2 image
        # note that the T1 and T2 images are already registered
        I_t2 = plt.imread('C:\\Users\\20203226\\Documents\\Boeken\\Q1\\8DC00\\data\\t2_demo.tif')
    ...
    ax2  = fig.add_subplot(132, xlim=(-np.pi, np.pi), ylim=(-40,-20))
