...
def ls_solve(A, b):
    ...
    w,res,rak,s = np.linalg.lstsq(A,b,rcond=None)
...
def ls_affine(X, Xm):
    ...
    A = np.transpose(Xm)
    b_x = np.transpose(X[0,:])
    b_y = np.transpose(X[1,:])
    w1_3,E1 = ls_solve(A,b_x) 
    w4_6,E2 = ls_solve(A,b_y)
    T = np.array([w1_3,w4_6,[0,0,1]])
    print(T)
...
def correlation(I, J):
    ...
    #------------------------------------------------------------------#
    C = np.transpose(u).dot(v) / (np.sqrt(np.transpose(u).dot(u))*np.sqrt(np.transpose(v).dot(v)))
...
def mutual_information(p):
    ...
    #------------------------------------------------------------------#
    #MI = sum(sum(p))*np.log(sum(sum(p))/sum(sum(p_I.dot(p_J))))
    MI=0 
    for i in range(1,p.shape[0]):
        for j in range(1,p.shape[1]):
            MI += p[i,j] * np.log(p[i,j]/(p_I[i-1,0]*p_J[0,j-1]))

    return MI

def Entropy(p):
    #log_p = np.log2(p)
    #E = -1* sum(np.transpose(p)*log_p)
    #print("\n"+str(np.transpose(p)*log_p)+"\n")
    #print("E"+str(E))
    E2 = 0
...
def mutual_information_e(p):
    ...
    #------------------------------------------------------------------#
    MI = Entropy(p_I) + Entropy(np.transpose(p_J)) - sum(Entropy(p))
    return MI
...
def affine_mi(I, Im, x, return_transform=True):
    ...
    p = joint_histogram(I, Im_t, NUM_BINS, [0, 255])
